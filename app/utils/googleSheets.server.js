...
import {
  ...
  testSheetConnection,
} from "../services/google.server.js"; // <- .js ajouté
...
function resolveAppField(order, appField) {
  ...
  switch (appField) {
    case "order.date": return o.createdAt || ord.createdAt || "";
    case "order.id": return ord.id || "";
    case "order.name": return ord.name || "";

    case "customer.name": return customer.name || "";
    case "customer.phone": return customer.phone || "";
    case "customer.city": return customer.city || "";
    case "customer.province": return customer.province || "";
    case "customer.country": return customer.country || "";
    case "customer.address": return customer.address || "";
    case "customer.notes": return customer.notes || "";

    case "cart.quantity": return cart.quantity ?? "";
    case "cart.productTitle": return cart.productTitle || "";
    case "cart.variantTitle": return cart.variantTitle || "";
    case "cart.offerName": return cart.offerName || "";
    case "cart.upsellName": return cart.upsellName || "";

    case "cart.subtotal": return cart.subtotal ?? "";
    case "cart.shipping": return cart.shipping ?? "";

    case "cart.total":
    case "cart.totalWithShipping": {
      if (cart.total != null) return `${cart.total} ${cart.currency || ""}`.trim();
      if (cart.totalCents != null) return `${cart.totalCents / 100} ${cart.currency || ""}`.trim();
      return "";
    }

    case "cart.currency": return cart.currency || "";

    default: return getDeep(o, appField) ?? "";
  }
}
...
export async function appendOrderToSheet({ shop, admin, order }) {
  ...
  if (!spreadsheetId) throw new Error("Aucun Spreadsheet ID configuré (cfg.sheet.spreadsheetId ou GOOGLE_SHEET_ID).");
  ...
  if (!accessToken) throw new Error("Aucun access token Google valide pour cette boutique.");
  ...
  if (!res.ok) throw new Error(json?.error?.message || json?.error || "Erreur Google Sheets (append)");
