...
// ✅ نفس منطق country ديال Section Form / Antibot
import { getCountries } from "../data/countryData";
...
/* ======================= Countries list (same as Form/Antibot) ======================= */
const APP_COUNTRIES = (() => {
  try {
    return (getCountries?.() || []).map((c) => ({
      code: String(c.code || "").toUpperCase(),
      label: c.label || String(c.code || "").toUpperCase(),
    }));
  } catch {
    return [];
  }
...
const COUNTRY_LABEL_MAP = (() => {
  const m = new Map();
  APP_COUNTRIES.forEach((c) => m.set(c.code, c.label));
  return m;
})();

const ALL_COUNTRY_CODES = (() => {
  if (APP_COUNTRIES.length) return APP_COUNTRIES.map((c) => c.code);
  // fallback (if getCountries ever empty)
  return ["MA"];
})();

function getCountryLabel(code) {
  const c = String(code || "").toUpperCase();
  return COUNTRY_LABEL_MAP.get(c) || c || "MA";
}
...
// ✅ إذا country كاين ف countryData ولكن ماعندوش provinces هنا → ما يطيحش
function getCountryDef(code) {
  const c = (code || "MA").toUpperCase();
  if (GEO_COUNTRIES[c]) return GEO_COUNTRIES[c];
  return { label: getCountryLabel(c), provinces: [] };
}
...
function defaultCfg() {
  const baseCountry =
    (ALL_COUNTRY_CODES.includes("MA") && "MA") || ALL_COUNTRY_CODES[0] || "MA";

  return {
    meta: { version: 2 },

    country: baseCountry,
    currency: baseCountry === "MA" ? "MAD" : "MAD",
    ...
    // Tarifs par province / wilaya - init pour tous pays du countryData
    provinceRates: Object.fromEntries(ALL_COUNTRY_CODES.map((c) => [c, []])),

    // Tarifs par ville / baladiya - init pour tous pays du countryData
    cityRates: Object.fromEntries(ALL_COUNTRY_CODES.map((c) => [c, []])),
...
export default function Section6Geo() {
  ...
  // ✅ countries options = نفس list ديال app (Form/Antibot)
  const countryOptions = useMemo(() => {
    const list =
      APP_COUNTRIES.length > 0
        ? APP_COUNTRIES
        : Object.keys(GEO_COUNTRIES).map((code) => ({
            code,
            label: GEO_COUNTRIES[code]?.label || code,
          }));
    return list.map((c) => ({ label: c.label, value: c.code }));
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (s) {
        ...
        if (parsed && typeof parsed === "object") {
          const allCountries = ALL_COUNTRY_CODES;
          ...
          allCountries.forEach((country) => {
            ...
            if (!updated.provinceRates[cc]) updated.provinceRates[cc] = [];
            if (!updated.cityRates[cc]) updated.cityRates[cc] = [];
          });
    ...
    // ✅ if no saved geo yet, sync from form country
    if (!hadLocal && formCountry) {
      const code = String(formCountry).toUpperCase().slice(0, 2);
      setCfg((old) => {
        const next = { ...old, country: code || old.country || "MA" };
        if (!next.provinceRates) next.provinceRates = {};
        if (!next.cityRates) next.cityRates = {};
        if (!next.provinceRates[next.country]) next.provinceRates[next.country] = [];
        if (!next.cityRates[next.country]) next.cityRates[next.country] = [];
        return next;
      });
    }
    ...
    async function loadFromServer() {
      try {
        ...
        allCountries.forEach((country) => {
          const cc = String(country).toUpperCase();
          if (!updatedGeo.provinceRates[cc]) updatedGeo.provinceRates[cc] = [];
          if (!updatedGeo.cityRates[cc]) updatedGeo.cityRates[cc] = [];
        });
        ...
        // sync localStorage
        try {
          window.localStorage.setItem("tripleform_cod_geo", JSON.stringify(updatedGeo));
        } catch {
          /* ignore */
        }
      } catch (e) {
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem("tripleform_cod_geo", JSON.stringify(cfg));
    } catch {
      /* ignore */
    }
  }, [cfg]);
    ...
    <PageShell t={t} onSave={saveGeo} saving={saving}>
      <div className="tf-editor">
        ...
        <div className="tf-rail">
          ...
          {/* Résumé livraison */}
          <div className="tf-rail-card">
            <div className="tf-rail-head">{t("section6.rail.summaryTitle")}</div>
            <div style={{ padding: 10 }}>
              <BlockStack gap="100">
                <InlineStack align="space-between">
                  <Text as="span">{t("section6.rail.type")}</Text>
                  <Badge>{cfg.isFree ? t("section6.rail.free") : t("section6.rail.paid")}</Badge>
                </InlineStack>
                ...
                {!cfg.isFree && (
                  <>
                    <InlineStack align="space-between">
                      <Text as="span">{t("section6.rail.priceBrackets")}</Text>
          ...
          <div className="tf-panel">
            ...
            <GroupCard title={t("section6.general.title")}>
              <Grid3>
                <Select
                  ...
                  options={[
                    { label: t("section6.general.freeOption"), value: "free" },
                    { label: t("section6.general.paidOption"), value: "paid" },
                  ]}
                />
                ...
                <Select
                  ...
                  onChange={setCountry}
                  options={[
                    { label: t("section6.general.countries.selectPlaceholder"), value: "" },
                    ...countryOptions,
                  ]}
              ...
              {!cfg.isFree && (
                <Grid3>
                  <Select
                    ...
                    options={[
                      { label: t("section6.general.modeProvince"), value: "province" },
                      { label: t("section6.general.modeCity"), value: "city" },
                      { label: t("section6.general.modePrice"), value: "price" },
                    ]}
                  />
                </Grid3>
              )}
            ...
            {!cfg.isFree && view === "province" && (
              ...
              >
                ...
                {/* إذا هاد country ماعندوش provinces ف GEO_COUNTRIES */}
                {provinceOptions.length === 0 ? (
                  <Text tone="subdued" as="p">
                    {t("section6.province.noDataForCountry")}
                  </Text>
                ) : (
                  <BlockStack gap="200">
                    {curProv.map((p) => (
                      <div className="row-card" key={p.id}>
                        <Grid3>
                          ...
                          <TextField
                            type="number"
                            label={t("section6.province.rateLabel", { currency: cfg.currency })}
                            value={String(p.rate)}
                    ...
                    <Button onClick={addProv}>{t("section6.buttons.addProvince")}</Button>
                  </BlockStack>
                )}
              </GroupCard>
            )}
            ...
            {!cfg.isFree && view === "city" && (
              ...
              >
                ...
                {provinceOptions.length === 0 ? (
                  <Text tone="subdued" as="p">
                    {t("section6.city.noDataForCountry")}
                  </Text>
                ) : (
                  <BlockStack gap="200">
                    {curCity.map((ci) => {
                      const cityOptions = getCityOptions(cfg.country, ci.province);
                      ...
                      return (
                        <div className="row-card" key={ci.id}>
                          <Grid3>
                            ...
                            <TextField
                              type="number"
                              label={t("section6.city.rateLabel", { currency: cfg.currency })}
                    ...
                    <Button onClick={addCity}>{t("section6.buttons.addCity")}</Button>
                  </BlockStack>
                )}
              </GroupCard>
            )}
            ...
            {!cfg.isFree && view === "price" && (
              <GroupCard title={t("section6.price.title")}>
                <Text tone="subdued" as="p">
                  {t("section6.price.description", { currency: cfg.currency })}
                </Text>
                <BlockStack gap="200">
                  {(cfg.priceBrackets || []).map((b) => (
                    <div className="row-card" key={b.id}>
                      <Grid3>
                        ...
                        <TextField
                          type="number"
                          label={t("section6.price.rateLabel", { currency: cfg.currency })}
                          value={String(b.rate)}
                  ...
                  <Button onClick={addBracket}>{t("section6.buttons.addBracket")}</Button>
                </BlockStack>
              </GroupCard>
            )}
            ...
            {view === "advanced" && (
              <GroupCard title={t("section6.advanced.title")}>
                <Grid3>
                  <TextField
                    type="number"
                    label={t("section6.advanced.defaultRate", { currency: cfg.currency })}
                  ...
                  <TextField
                    type="number"
                    label={t("section6.advanced.freeThreshold", { currency: cfg.currency })}
                  ...
                  <TextField
                    type="number"
                    label={t("section6.advanced.minOrderAmount", { currency: cfg.currency })}
                  ...
                  <TextField
                    type="number"
                    label={t("section6.advanced.codExtraFee", { currency: cfg.currency })}
