...
/* ============================== FIX: Polaris Icon source resolver ============================== */
...
function Section1FormsLayoutInner() {
  ...
  const setUi = (p) => setConfig((c) => ({ ...c, ui: { ...c.ui, ...p } }));
...
function OutletEditor() {
  const { config, setCartT, setForm, setUiT, setField, setDesign, setBehav, setUi, setFieldsOrder, t } =
    useForms();
        ...
        <div className="tf-panel">
          ...
          {sel === "options" && (
            <GroupCard title={t("section1.group.options.title")}>
              <BlueSection title="Coupon & Variants" defaultOpen>
                <InlineStack gap="200" blockAlign="center">
                  <Checkbox
                    label="Enable coupon field"
                    checked={config.ui?.enableCouponField || false}
                    onChange={(v) => setUi({ enableCouponField: v })}
                  />
                  <Checkbox
                    label="Enable variant selector"
                    checked={config.ui?.enableVariantSelector || false}
  ...
  const renderCouponField = () => {
    ...
    return (
      <div style={{ marginTop: 12, display: "flex", gap: 10, alignItems: "end" }}>
        ...
        <button type="button" style={btnMini}>
          {config.uiTitles.applyCoupon || "Apply"}
        </button>
      </div>
    );
  };

  // ✅ Ajout du sélecteur de variante si activé
  const renderVariantSelector = () => {
    if (!config.ui?.enableVariantSelector) return null;

    const variants = [
      { id: "1", title: "Variant 1 - Small" },
      { id: "2", title: "Variant 2 - Medium" },
      { id: "3", title: "Variant 3 - Large" },
    ];
